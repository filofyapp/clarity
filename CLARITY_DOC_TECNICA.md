...
FECHA: 17/04/2026
QUE SE CAMBIO: Fix Auditoría — Falsos desvíos + Ausentes + Resumen mensual/diario + RLS text[]
POR QUE: (1) La migración 036 fallaba con "operator does not exist: text[] @> jsonb" porque usuarios.roles es text[], no jsonb. (2) Casos en estado en_consulta_cia aparecían como desvíos cuando en realidad el problema es de la compañía (el perito sí fue). (3) Las inspecciones ausentes (perito fue pero asegurado no estaba) no contaban como presenciales. (4) El resumen del informe WhatsApp mezclaba totales mensuales (93 presenciales) con denominador diario (15 inspecciones), resultando en porcentajes absurdos (620%).
COMO: (1) RLS: Cambiado de `usuarios.roles @> '["admin"]'::jsonb` a `'admin' = ANY(usuarios.roles)` en las 5 políticas de la migración 036. (2) ESTADOS_POST_INSPECCION: Agregados en_consulta_cia, ip_reclamada_perito, esperando_respuesta_tercero al array de estados que NO son desvío. (3) Ausentes: Agregado campo tipo_inspeccion a CasoAuditoria. En determinarTipoInspeccion(), si tipo_inspeccion === 'ausente' → retorna 'presencial'. Agregado tipo_inspeccion a TODAS las queries SQL (.select) en actions.ts y cron/route.ts. (4) Resumen WhatsApp: Separado total_inspecciones_dia (diario) de total_inspecciones_mes (mensual). El resumen ahora muestra "Inspecciones programadas hoy: 15" y "Total del mes: 115 (Presenciales: 96 (96%) | Remotas: 7 (7%))".
ARCHIVOS AFECTADOS: supabase/migrations/036_auditoria_modulo.sql (5 políticas RLS corregidas), src/lib/auditoria-engine.ts (ESTADOS_POST_INSPECCION ampliado, CasoAuditoria.tipo_inspeccion, determinarTipoInspeccion con ausente, InformeAuditoriaDatos.total_inspecciones_mes, generarTextoWhatsApp resumen corregido), src/app/(dashboard)/auditoria/actions.ts (tipo_inspeccion en queries y CasoAuditoria, total_inspecciones_mes), src/app/api/cron/informe-auditoria/route.ts (tipo_inspeccion en queries, total_inspecciones_mes)
EFECTOS COLATERALES: Si hay informes ya generados con el formato viejo, sus textos guardados en contenido_whatsapp no se actualizan retroactivamente (solo los nuevos). Los scores podrían cambiar al recalcular porque ahora en_consulta_cia/ip_reclamada/esperando_tercero ya no cuentan como desvío.
TESTEADO: TypeScript --noEmit OK (0 errores).

FECHA: 17/04/2026
QUE SE CAMBIO: Umbrales de Score Configurables — Admin puede personalizar qué score es Excelente/Bueno/Regular/Crítico.
POR QUE: Los umbrales hardcodeados (90/70/50) no permitían al administrador ajustarlos según el contexto operativo del estudio.
COMO: (1) auditoria-engine.ts: Creada interfaz UmbralesScore {excelente, bueno, regular} y constante UMBRALES_SCORE_DEFAULT (90/70/50). getScoreColor() y getScoreLabel() ahora aceptan parámetro opcional UmbralesScore con default backward-compatible. (2) actions.ts: Agregadas server actions getUmbralesScore() — lee de tabla configuracion clave 'umbrales_score_auditoria' — y setUmbralesScore() — upsert con validación excelente > bueno > regular > 0. (3) AuditoriaPanel.tsx: Agregado estado umbrales + fetch al montar. Botón ⚙️ en header abre componente UmbralesSettings con 3 inputs numéricos + color dots + botón guardar. Todos los getScoreColor/getScoreLabel pasan umbrales como parámetro (PeritoCard, VistaIndividual, tabla histórica, gráfico SVG).
ARCHIVOS AFECTADOS: src/lib/auditoria-engine.ts (UmbralesScore, UMBRALES_SCORE_DEFAULT, getScoreColor/getScoreLabel con parámetro), src/app/(dashboard)/auditoria/actions.ts (getUmbralesScore, setUmbralesScore), src/components/auditoria/AuditoriaPanel.tsx (+Settings icon, +UmbralesSettings component, umbrales state, prop drilling)
EFECTOS COLATERALES: Si no existe la clave 'umbrales_score_auditoria' en la tabla configuracion, se usan los defaults (90/70/50). No requiere migración SQL — usa la tabla configuracion existente. La tabla configuracion ya tiene RLS permisiva para lectura por admin.
TESTEADO: TypeScript --noEmit OK (0 errores).

FECHA: 17/04/2026
QUE SE CAMBIO: Fix Auditoría — Casos futuros en denominador + corte 18hs AR + desvíos resueltos
POR QUE: (1) El denominador del score incluía casos con fecha_ip en el futuro (coordinados para la semana que viene), bajando artificialmente el score. Emiliano mostraba 41/48 (85%) cuando debería ser 41/41 (100%). (2) No había concepto de "corte del día" — el deadline era medianoche UTC en vez de 18:00 Argentina. (3) Los desvíos pasados que se resolvieron tarde (ej: coordinado Apr 7, completado Apr 9) no se mostraban ni penalizaban — solo se contaban los activos.
COMO: (1) getFechaCorteStr(): nueva función que calcula la fecha de corte basada en 18:00 Argentina (UTC-3). Antes de las 18hs AR, los casos de hoy aún tienen tiempo (corte=ayer). Después de las 18hs, los de hoy ya deben estar completados (corte=hoy). (2) detectarDesvios(): cambiado de fechaRef:Date a fechaCorteStr:string. Casos con fechaIP > fechaCorte se ignoran. Deadline = fechaIP 21:00 UTC (18:00 AR). (3) detectarDesviosResueltos(): nueva función que detecta casos completados DESPUÉS de su deadline. Usa fecha_inspeccion_real o historial_estados como fallback. (4) calcularDatosAuditoriaPeriodo(): acepta fechaCorteStr e historialPostInspeccion. El denominador (casos_totales) ahora es casos ELEGIBLES = casos con fechaIP <= corte. (5) PeritoResumen.desvios_resueltos: nuevo campo. Score: penalidad incluye activos + resueltos. (6) CasoAuditoria.fecha_inspeccion_real: nuevo campo. (7) UI: AuditoriaPanel muestra sección "Completados con demora" con color ámbar separada de desvíos activos (rojo). Cards muestran ambos counts. Desglose muestra "Desvíos activos" y "Desvíos resueltos" por separado. (8) WhatsApp: sección ⚠️ "Completadas con demora" separada de ❌ "No realizadas". (9) Cron: actualizado con misma lógica.
ARCHIVOS AFECTADOS: src/lib/auditoria-engine.ts (getFechaCorteStr, detectarDesviosResueltos, CasoAuditoria.fecha_inspeccion_real, PeritoResumen.desvios_resueltos, calcularDatosAuditoriaPeriodo signature change, WhatsApp text), src/app/(dashboard)/auditoria/actions.ts (getFechaCorteStr import, fecha_inspeccion_real en queries, historialPostInsp query, ESTADOS_POST_INSPECCION_SET), src/components/auditoria/AuditoriaPanel.tsx (desvios_resueltos en Card + VistaIndividual), src/app/api/cron/informe-auditoria/route.ts (mismas queries + engine API)
EFECTOS COLATERALES: (1) Los scores van a cambiar significativamente después de este fix — especialmente subir para peritos con muchos casos futuros. (2) Los desvíos resueltos solo se detectan si hay fecha_inspeccion_real o historial_estados con transición a post-inspección. Casos antiguos sin historial no se detectan como resueltos. (3) El concepto de "18:00 Argentina" está hardcodeado como UTC-3 (Argentina no usa horario de verano).
TESTEADO: TypeScript --noEmit OK (0 errores).

FECHA: 20/04/2026
QUE SE CAMBIO: Fix Informe Campo — "duplicate key value violates unique constraint idx_informe_campo_caso"
POR QUE: Los peritos de calle no podían guardar el informe de inspección presencial. Al intentar guardar, el sistema mostraba "Error al guardar el informe: duplicate key value violates unique constraint idx_informe_campo_caso". Esto ocurre cuando el perito reenvía el formulario (doble click, retry por red lenta, o caso que vuelve a ip_coordinada y se reinspecciona).
COMO: guardarInspeccionCampo() en inspeccion-campo/[casoId]/actions.ts: Cambiado .insert() a .upsert() con { onConflict: "caso_id" }. Si ya existe un registro para ese caso_id, se actualiza en vez de intentar insertar uno nuevo y fallar por el UNIQUE INDEX.
ARCHIVOS AFECTADOS: src/app/(dashboard)/inspeccion-campo/[casoId]/actions.ts (insert → upsert con onConflict)
EFECTOS COLATERALES: Si un perito reenvía el informe, se sobreescribe el anterior. No hay pérdida de datos — la última versión es la correcta. La función actualizarInspeccionCampo() ya usaba .update(), y no es afectada.
TESTEADO: TypeScript --noEmit OK (0 errores).

BUG RECURRENTE — REGLA PARA DESARROLLADORES:
PROBLEMA: Usar .insert() con tablas que tienen UNIQUE INDEX provoca "duplicate key" si el registro ya existe (reenvío, retry, reinspección).
CAUSA: No anticipar que el usuario puede enviar el formulario más de una vez para el mismo caso.
SOLUCION: SIEMPRE usar .upsert({ ... }, { onConflict: "campo_unique" }) cuando la tabla tiene un UNIQUE INDEX y el registro podría existir. Tablas conocidas con UNIQUE en caso_id: informe_inspeccion_campo (idx_informe_campo_caso).
FECHA: 20/04/2026
NO REPETIR: Antes de hacer .insert() en CUALQUIER tabla, verificar si tiene UNIQUE INDEX. Si lo tiene, usar .upsert() con onConflict.

FECHA: 21/04/2026
QUE SE CAMBIO: BUG-028 — Fix BotonAusente: crash al confirmar ausencia (3 causas: sin compresión de imagen, sin conversión HEIC, permisos con campo legacy `rol`).
POR QUE: Emiliano De Lia (perito de calle) reportó que al intentar registrar una ausencia y confirmar con la foto, la app mostraba un error. El BotonAusente era el UNICO componente de carga de fotos que NO comprimía las imágenes antes de enviarlas al Server Action. Las fotos de celular pesan 3-12MB y se enviaban crudas como FormData. Esto causaba: (1) Exceder el bodySizeLimit de 10MB en algunos dispositivos con cámaras de alta resolución, o timeout en conexiones 4G lentas en campo. (2) Fotos HEIC de iPhone (galería) no se convertían a JPEG, causando que el canvas de Supabase Storage no las procesara correctamente. (3) El check de permisos en marcarInspeccionAusente() usaba `usuario.rol` (string legacy) en vez de `usuario.roles` (array), un bug recurrente documentado en BUG-027 que puede causar falsos negativos de permisos para usuarios con multi-role o campo legacy desactualizado.
COMO: (1) BotonAusente.tsx: Agregado pipeline completo de procesamiento de imagen (compressImage + convertirSiEsHeic + procesarImagen) — misma lógica de WizardCaptura e InspeccionCampoWizard. Fotos se comprimen a 1920px/Q80 (~300-500KB) antes de enviar. Nuevo estado `fotoComprimida` para separar foto original (preview) de foto procesada (upload). Nuevo estado `isProcessing` con overlay de "Procesando..." y bloqueo del botón de confirmación mientras procesa. Memory cleanup con URL.revokeObjectURL en close/replace. (2) actions.ts marcarInspeccionAusente: Cambiado `.select("rol")` a `.select("rol, roles")`. Check de permisos ahora usa `roles.includes("admin")` con fallback al campo legacy, siguiendo el patrón establecido en BUG-027.
ARCHIVOS AFECTADOS: src/components/casos/BotonAusente.tsx (reescrito), src/app/(dashboard)/casos/[id]/actions.ts (fix permisos)
EFECTOS COLATERALES: Ninguno. Las fotos de ausencia ahora se comprimen (~80% reducción de tamaño) y se convierten de HEIC a JPEG. El pipeline de compresión tiene fallback seguro (si falla, envía la foto original). La experiencia visual no cambia para el usuario — solo se agrega un breve indicador "Procesando..." mientras se comprime.
TESTEADO: TypeScript tsc --noEmit pasa con 0 errores.

### BUG-028: BotonAusente crash al confirmar ausencia — foto sin comprimir + HEIC + permisos legacy (RESUELTO)
- PROBLEMA: El perito de calle (Emiliano De Lia) no podía registrar una ausencia. Al confirmar con la foto, la app mostraba un error. El perito no pudo detallar qué error era exactamente.
- CAUSA: Triple falla en BotonAusente: (1) Era el UNICO componente de upload de fotos sin pipeline de compresión — WizardCaptura e InspeccionCampoWizard comprimen a ~300-500KB, pero BotonAusente enviaba la foto cruda (3-12MB). Con fotos de alta resolución o conexiones 4G lentas, el Server Action fallaba por bodySizeLimit o timeout. (2) No tenía conversión HEIC→JPEG para fotos de iPhone desde galería. (3) marcarInspeccionAusente() usaba `usuario.rol` (string legacy) para verificar admin, en vez de `usuario.roles` (array) — patrón de BUG-027 recurrente.
- SOLUCION: (1) Agregado pipeline compressImage + convertirSiEsHeic + procesarImagen idéntico al de WizardCaptura. (2) Separación de estado: `foto` (original para preview), `fotoComprimida` (procesada para upload). (3) Estado `isProcessing` con indicador visual y bloqueo de botón. (4) Fix de permisos: `.select("rol, roles")` + `roles.includes("admin")`.
- FECHA: 21/04/2026
- NO REPETIR: TODOS los componentes que suben fotos DEBEN usar el pipeline de compresión (compressImage/procesarImagen). NUNCA enviar fotos crudas de cámara a un Server Action. La lista de componentes con compresión correcta: WizardCaptura ✓, InspeccionCampoWizard ✓, BotonAusente ✓ (ahora). Si se crea un nuevo componente de upload de fotos, COPIAR el pipeline de compresión.
